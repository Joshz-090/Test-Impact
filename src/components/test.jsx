...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    // Three.js setup for 3D particles
    ...
    const ctx = gsap.context(() => {
      ...
      // Hero content animation with 3D effect
  ...
  return (
    <section
      className="relative h-[100vh] w-full overflow-hidden flex items-center justify-center hero-section bg-black/50"
      role="banner"
      aria-label="Hero section with 3D and video background"
      ref={containerRef}
    >
      <style>
        {`
          ...
          .floating-element:nth-child(2) div {
            animation-delay: 1s;
          }
          .hero-headline:hover {
      ...
      {/* 3D Canvas Background (primary when no reduced motion) */}
      ...
      {/* Video Background (fallback or when Three.js is disabled) */}
      <video
        ref={videoRef}
        className={`absolute inset-0 w-full h-[100vh] object-cover ${!prefersReducedMotion && !videoError ? 'opacity-30' : 'opacity-70'}`}
        autoPlay
        loop
        muted
        playsInline
      ...
      {/* Fallback Image (outside video tag) */}
      {videoError && (
        <img
      ...
      {/* Gradient Overlay */}
      <div
        className="absolute inset-0 bg-gradient-to-b from-[rgba(10,30,90,0.7)] to-[rgba(0,0,0,0.95)] backdrop-blur-[2px]"
        aria-hidden="true"
      />
      ...
      <div className="hero-content relative z-10 text-center text-white px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        >
          ...
          <span className="relative inline-block group cursor-pointer">
            ...
            <span
              className="absolute inset-0 w-0 text-[#f1c84c] overflow-hidden whitespace-nowrap transition-all duration-500 group-hover:w-full"
            >
              Impact Production
            </span>
            <span
              className="absolute bottom-0 left-0 h-0.5 bg-[#D4AF37] transition-all duration-500 ease-out w-0 group-hover:w-full origin-left"
            ></span>
          </span>
        </h1>
        ...
        <p
          ref={subtitleRef}
          className="text-base sm:text-lg lg:text-xl opacity-90 max-w-2xl mx-auto leading-relaxed mb-8 italic transition-all duration-1000"
          id="hero-description"
        >
        ...
        <div className="floating-element absolute top-16 left-8">
          <div className="w-5 h-5 bg-[#D4AF37] rounded-full shadow-[0_0_12px_rgba(212,175,55,0.5)]"></div>
        </div>
        <div className="floating-element absolute bottom-24 right-8">
          <div className="w-6 h-6 bg-[#D4AF37] rounded-full shadow-[0_0_12px_rgba(212,175,55,0.5)]"></div>
        </div>
